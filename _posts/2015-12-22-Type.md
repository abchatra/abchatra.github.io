...
Above information needs to be retrieved when the script wants to get or set the property. Note (1) is shareable among multiple objects. (2) & (3) are specific to individual dynamic objects. A typical way to store above information is in a form of a property map and a slot array.
...
Type is common across all the objects created from constructor *Point*. Even if you create a million points, you just need one type to represent them all. 
