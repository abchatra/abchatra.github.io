...
starwars4(one, two);
```
The assumption here is functions *starwars* doesn't mutate objects, it just access properties off them. Its optimal for objects *one* and *two* point to the exact same type when they are constructed. Both these objects end up with property x and y as well. So they can share the same type at the end when the function *starwars4* is invoked. In between these two states we can see that objects *one* and *two* differ in what they contain. At the call site of *starwars2* object *one* has x & y but object *two* has none at all. How do you make sure all the objects with same properties end up with the exact type? 

Successor type in typehanlder comes to rescue. Each typehandler holds a successor type which tracks what type transition to take if objects get added a new property (also known as promoting type). Following diagram tries to illustrate the same. 
