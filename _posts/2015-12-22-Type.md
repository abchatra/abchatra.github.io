...
To easily understand this in Chakra codebase let us take a look at  [SimpleTypeHandler](https://github.com/Microsoft/ChakraCore/blob/master/lib/Runtime/Types/PathTypeHandler.h#L207). SimpleTypeHanlder has a property map named [typePath](https://github.com/Microsoft/ChakraCore/blob/master/lib/Runtime/Types/PathTypeHandler.h#L15) in the base class which has a [tiny dictionary] (https://github.com/Microsoft/ChakraCore/blob/master/lib/Runtime/Types/TypePath.h#L73) to map a propertyId (synonymous to property name) to a slot number. It also consists of successorTypeWeakRef which holds the pointer to next type. SimpleTypeHanlder holds  pointer to only one successor. [PathTYpeHandler] (https://github.com/Microsoft/ChakraCore/blob/master/lib/Runtime/Types/PathTypeHandler.h#L243) holds PropertySuccessorsMap to multiple successors. 

###Summary

We looked at why objects have types and how it shares data. We also looked at briefly at how type handle stores a pointer to successor types. There are various other type handlers which share data differently or not share at all. It is an exercise to the reader to figure out the need for all these. Ask me here if you have specific questions. 

In my opinion biggest advantage of types is inline cache. We will look into inline cache in the next post. Feel free to ask me more questions on this topic in the comments. 
